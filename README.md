...
On Cloudflare Workers, Cron Triggers warm the shared NCR news pool every 2 hours, plus **21:00 UTC** (5:00 AM in Manila). Cron refreshes RSS only and merges it into the existing KV pool, so extracted Palace bodies survive a Google-link refresh and survive dropping out of the feed. Cron does not spend Browser Rendering on article decode (that wait stays on the first request that still needs a Palace/work body). Requests are cached in KV for 7 days with background revalidation. The "As of" time is when headlines were fetched, not when the page was opened.
